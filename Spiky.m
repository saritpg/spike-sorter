...
function Spiky_OpeningFcn(h, eventdata, handles, varargin)

handles.output = h;
guidata(h, handles);

% -- Outputs from this function are returned to the command line.-------------
function varargout = Spiky_OutputFcn(h, eventdata, handles)
...
spikedata=[];
topt=[];
for j=3:1:(size(newdata,1)-2)
    if (newdata(j+2,2)<newdata(j,2)) & (newdata(j-2,2)<newdata(j,2)) & (newdata(j,2)>topthresh)
       topt=[topt;newdata(j,1),newdata(j,2),j];
    end
end

toppt=[];
for i=1:(size(topt,1)-1)
    if topt(i,1)+0.0003<topt(i+1,1)
       toppt=[toppt;topt(i,1),topt(i,2),topt(i,3)];
    end
end


for k=1:(size(toppt,1)-1)
    for j=3:(size(newdata,1)-59)
        if toppt(k,1)==newdata(j,1)
           n=toppt(k+1,3); 
           bottompt=[1,meanmaxmin];
            for i=j:n
                if newdata(i,2)<bottompt(1,2) & newdata(i+2,2)>newdata(i,2)& newdata(i-2,2)>newdata(i,2) & newdata(i,2)<bottomthresh
                   bottompt=[newdata(i,1),newdata(i,2)];
                end
            end 
            if bottompt(1,2)~=meanmaxmin
               spikedata=[spikedata;toppt(k,1),toppt(k,2);bottompt(1,1),bottompt(1,2)];
            end
        end
...
for i=1:size(totalamp,1)
    ...
    elseif (totalamp(i,1)>0.75*lengthspikeC)|(negativeamp(i,1)>0.70*negativeampC) 
        nC=nC+1;
...
for i=1:size(totalamp,1)
    if (totalamp(i,1)>0.70*lengthspikeA)|(negativeamp(i,1)>0.70*negativeampA)
        nA=nA+1;
        ampA=[ampA;totalamp(i,1)];  
    elseif (negativeamp(i,1)>0.80*negativeampB)
        nB=nB+1;
        ampB=[ampB;totalamp(i,1)];
    end
end
